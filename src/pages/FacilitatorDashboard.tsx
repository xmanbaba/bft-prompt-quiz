...
import { LogOut, RefreshCw, Download, Plus, X } from 'lucide-react';
...
export default function FacilitatorDashboard() {
  ...
  const [newSession, setNewSession] = useState({ name: '', industry_id: '', framework_ids: [] as string[], pass_threshold: 3.5, mcq_count: 8, scenarios_per_framework: 2, require_email: false, session_date: '' });
  const [creating, setCreating] = useState(false);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    supabase.auth.getUser().then(({ data }) => {
      if (!data.user) navigate('/facilitator');
    });
    loadLookups();
    loadSessions();
  }, [navigate]);
  ...
  function exportCSV() {
    ...
    const rows = [['Participant', 'Email', 'Status', 'MCQ Score', 'Item', 'Framework', 'Category', 'Score', 'Max', 'AI Flag', 'Missed Elements', 'Answer']];
      ...
      if (!myResps.length) {
        ...
        myResps.forEach(r => {
          rows.push([p.name, p.email ?? '', p.status, '', r.scenario_id, r.framework_acronym,
            (r.scenarios as { category?: string })?.category ?? '', String(r.total_score ?? ''), String(r.max_score ?? ''),
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-blue-dark text-white px-6 py-4 flex justify-between items-center sticky top-0 z-20 shadow-lg">
        <div>
          ...
          {selectedSession && <div className="text-xs text-white/60 mt-0.5">Code: {selectedSession.code} · {selectedSession.industry?.name}</div>}
        </div>
        <div className="flex gap-2 flex-wrap">
          {selectedSession && <button onClick={() => { setSelectedSession(null); setTab('sessions'); }} className="bg-white/15 border border-white/30 text-white text-xs font-semibold px-3 py-1.5 rounded-md">← Sessions</button>}
          {selectedSession && <button onClick={exportCSV} className="bg-white/15 border border-white/30 text-white text-xs font-semibold px-3 py-1.5 rounded-md flex items-center gap-1"><Download className="w-3 h-3" />CSV</button>}
          {selectedSession && <button onClick={() => loadSessionData(selectedSession)} className="bg-white/15 border border-white/30 text-white text-xs font-semibold px-3 py-1.5 rounded-md flex items-center gap-1"><RefreshCw className="w-3 h-3" />Refresh</button>}
          <button onClick={handleSignOut} className="bg-white/15 border border-white/30 text-white text-xs font-semibold px-3 py-1.5 rounded-md flex items-center gap-1"><LogOut className="w-3 h-3" />Sign out</button>
        </div>
      </div>
        ...
        {!selectedSession && (
          <>
            <div className="flex justify-between items-center mb-4">
              ...
              <button onClick={() => setShowCreateSession(true)} className="bg-blue text-white text-xs font-bold px-4 py-2 rounded-lg flex items-center gap-1 hover:opacity-90">
            ...
            {loading ? (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {sessions.map(s => (
                  <button key={s.id} onClick={() => loadSessionData(s)} className="card text-left hover:border-blue hover:shadow transition-all">
                    <div className="font-bold text-sm text-gray-800 mb-1">{s.name}</div>
                    <div className="text-xs text-gray-500 mb-2">{(s.industry as Industry)?.name} · {s.code}</div>
          ...
          <>
            ...
            <div className="flex border-b-2 border-gray-200 mb-5 overflow-x-auto">
              {(['participants', 'responses'] as const).map(t => (
                <button key={t} onClick={() => setTab(t)} className={`px-5 py-2.5 text-sm font-bold capitalize border-b-2 -mb-0.5 whitespace-nowrap transition-colors ${tab === t ? 'border-blue text-blue-dark' : 'border-transparent text-gray-500'}`}>
                  {t}
                </button>
              ))}
            </div>
            ...
            {tab === 'participants' && (
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
                {participants.length === 0 && <div className="col-span-full text-center text-gray-500 py-8">No participants yet. Share code: <strong>{selectedSession.code}</strong></div>}
                {participants.map(p => {
                  const myResps = responses.filter(r => r.participant_id === p.id && r.scoring_status === 'done');
                  const total = myResps.reduce((s, r) => s + (r.total_score ?? 0), 0);
                  const max = myResps.reduce((s, r) => s + (r.max_score ?? 0), 0);
                  ...
                  return (
                    <button key={p.id} onClick={() => { setSelectedParticipant(selectedParticipant === p.id ? null : p.id); setTab('responses'); }}
                  ...
                  <div key={r.id} className="card mb-4">
                    <div className="flex justify-between items-start flex-wrap gap-2 mb-3">
                      ...
                      <div className="flex gap-2 items-center">
                        ...
                        <button onClick={() => setProjecting(r)} className="text-xs font-bold text-blue border border-blue px-3 py-1 rounded-lg hover:bg-blue-light">Project</button>
                      </div>
                    </div>
            ...
            <div className="flex flex-col gap-2">
              {Object.entries(projecting.element_scores).map(([el, data]) => (
                <div key={el} className="flex justify-between bg-white/6 rounded-lg px-4 py-3 text-sm">
                  <span>{el}</span>
                  <span>{data.score >= 3 ? '✓' : '✗'} {data.note}</span>
                </div>
              ))}
              <div className="flex justify-between bg-white/6 rounded-lg px-4 py-3 text-sm font-bold">
                <span>Total</span>
                <span>{projecting.total_score} / {projecting.max_score}</span>
              </div>
            </div>
          )}
        </div>
        ...
        <div className="fixed inset-0 bg-black/50 z-40 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl p-6 w-full max-w-lg max-h-[90vh] overflow-y-auto shadow-2xl">
            ...
            <div className="mb-3">
              ...
              <input className="input-field" placeholder="e.g. WARIF Staff Training — July 2026" value={newSession.name} onChange={e => setNewSession(s => ({ ...s, name: e.target.value }))} />
            </div>
            <div className="mb-3">
              <label className="label">Industry</label>
              <select className="input-field" value={newSession.industry_id} onChange={e => setNewSession(s => ({ ...s, industry_id: e.target.value }))}>
                <option value="">Select industry...</option>
                {industries.map(i => <option key={i.id} value={i.id}>{i.name}</option>)}
              </select>
            </div>
            <div className="mb-3">
              <label className="label">Frameworks to test (select all that apply)</label>
            ...
            <div className="grid grid-cols-3 gap-3 mb-3">
              <div>
                ...
                <input className="input-field" type="number" min="1" max="5" step="0.5" value={newSession.pass_threshold} onChange={e => setNewSession(s => ({ ...s, pass_threshold: Number(e.target.value) }))} />
              </div>
              <div>
                <label className="label">MCQ count</label>
                <input className="input-field" type="number" min="4" max="15" value={newSession.mcq_count} onChange={e => setNewSession(s => ({ ...s, mcq_count: Number(e.target.value) }))} />
              </div>
              <div>
                <label className="label">Scenarios/framework</label>
                <input className="input-field" type="number" min="1" max="4" value={newSession.scenarios_per_framework} onChange={e => setNewSession(s => ({ ...s, scenarios_per_framework: Number(e.target.value) }))} />
              </div>
            </div>
            <div className="mb-3">
              <label className="label">Session date (optional)</label>
              <input className="input-field" type="date" value={newSession.session_date} onChange={e => setNewSession(s => ({ ...s, session_date: e.target.value }))} />
            </div>
            <div className="flex items-center gap-2 mb-5">
              <input type="checkbox" id="req-email" checked={newSession.require_email} onChange={e => setNewSession(s => ({ ...s, require_email: e.target.checked }))} className="accent-blue" />
              <label htmlFor="req-email" className="text-sm text-gray-700">Require participants to enter their email address</label>
            </div>

            <button className="btn-primary" onClick={createSession} disabled={creating || !newSession.name || !newSession.industry_id || newSession.framework_ids.length === 0}>
